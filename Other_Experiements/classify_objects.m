function overall_acc = classify_objects(data_dir, t_start, noise_level, n_signals, peak_offset, win_length)
...
%   classify_objects(data_dir, t_start, noise_level, n_signals, peak_offset, win_length)
%
%   Required inputs:
...
%                    (used as a fixed floor, or as a lower bound on the
%                    per-signal window start when peak_offset is supplied)
...
%     win_length   - length of the GLRT window in time units. Only used
%                    together with peak_offset. The window applied is:
%                        t_start_i  <= t  <  t_start_i + win_length
%                    Omit or pass [] to use all samples from t_start_i
...
if nargin < 6 || isempty(win_length)
    win_length = [];   % no upper bound on window
end
...
% -------------------------------------------------------------------------
fprintf('Settings: noise_level=%.2g | t_start=%.4g | n_signals=%s | peak_mode=%s | win_length=%s\n\n', ...
        noise_level, t_start, num2str(n_signals), mat2str(use_peak_mode), num2str(win_length));
...
for true_class = 1:num_classes
    ...
    for row = 1:n
        ...
        % --- select the time window t >= t_start_row (and < t_end_row) ----
        if ~isempty(win_length)
            t_end_row = t_start_row + win_length;
            time_mask = ts >= t_start_row & ts < t_end_row;
        else
        ...
        if predicted_class == true_class
            ...
            fprintf("True Class " + num2str(true_class) + " mis predicted as " + num2str(predicted_class) + " at row " + num2str(row) + "\n")
        end
    end
